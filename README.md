...
5. 安装扩展：`pip install sphinx-design`
---
...
* 启用扩展（用来生成`.nojekyll`文件）：`extensions = ["sphinx.ext.githubpages",]`
* 启用扩展（`QA.rst`需要下拉功能）：`extensions = [ "sphinx_design",]`
...
* [多语言支持](#多语言支持)：
  * ~~默认语言：`language = 'en'`~~
  * 配置“翻译文件”的相对路径：`locale_dirs = ['locale/']`
  * 保持`.po`文件按文档结构组织：`gettext_compact = False`
* 多语言切换（通过注入模版变量，另见`_templates/layout.html`）：
  ```python
  html_context = {
      'languages': {
        'en': {'name': 'English'},
...
4. 清理`#~`过期条目：
   ``` bash
   for f in locale/zh/LC_MESSAGES/*.po; do
      msgattrib --no-obsolete -o "$f" "$f"
   done
   ```
5. 自动构建命令：`sphinx-autobuild -b html -D language='zh' source build/zh --port 8000`
6. 编辑翻译文件，实时预览（例如：`source/locale/zh/LC_MESSAGES/index.po`）：
   ```
